...
from transformers import LogitsProcessor # 自己加的

class SafeLogitsProcessor(LogitsProcessor): # 自己加的
...
class MiniGPTBase(BaseModel):
    ...
    def get_context_emb(self, prompt, img_list):
        ...
        mask_embs = []
        for i, seg_emb in enumerate(seg_embs[:-1]):  # 遍历除了最后一个之外的文本嵌入
            mask_embs.append(seg_emb)  # 添加文本嵌入
            if i < len(img_list):  # 确保有对应的图像嵌入存在
                zero_mask = torch.zeros_like(img_list[i])  # 为对应的图像嵌入创建一个全零向量
                mask_embs.append(zero_mask)  # 添加全零图像嵌入掩码
        mask_embs.append(seg_embs[-1])  # 加上列表中的最后一个文本嵌入
        mask_embs = torch.cat(mask_embs, dim=1)

        return mixed_embs, mask_embs
    
    def get_inputs_ids(self, prompt, img_list):
        device = img_list[0].device
        prompt_segs = prompt.split('<ImageHere>')
        ...
        mixed_ids = torch.cat(seg_tokens, dim=1)
        
        # return mixed_ids, seg_tokens[-1].shape[1], seg_tokens
        return mixed_ids, seg_tokens
    
    
# print(chat_state.get_prompt())
# Give the following image: <Img>ImageContent</Img>. You will be able to see the image once I provide it to you. 
# Please answer my questions.<s>[INST] <Img><ImageHere></Img> Describe the content of the image. [/INST] 
    ...
    def generate(
        ...
        batch_embs = [self.get_context_emb(text, img_list)[0] for text, img_list in zip(texts, image_lists)]
        ...
        with self.maybe_autocast():
            outputs = self.llama_model.generate(
                ...
                logits_processor=[SafeLogitsProcessor()],  # 自己加的
