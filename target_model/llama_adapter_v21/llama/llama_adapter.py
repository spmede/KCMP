...
class LLaMA_adapter(nn.Module):
    ...
    def __init__(self, llama_ckpt_dir, llama_tokenizer,
        ...
        # for name, param in self.named_parameters():  # 注释，避免打印太多信息
...
def load(name, llama_dir, llama_type="7B", device="cuda" if torch.cuda.is_available() else "cpu", download_root='ckpts', max_seq_len=512, phase="finetune", max_batch_size=32):
    if name in _MODELS:
    ...
    llama_ckpt_dir = os.path.join(llama_dir, llama_type)  # 源代码
    # llama_ckpt_dir = llama_dir  # 自己改的
    ...
    model = LLaMA_adapter(
        ...
        max_seq_len=512, 
        max_batch_size=max_batch_size,  # 改动 batch size
        clip_model='ViT-L/14',
        v_embed_dim=768, v_depth=8,
        v_num_heads=16, v_mlp_ratio=4.0,
        query_len=10, query_layer=31,
