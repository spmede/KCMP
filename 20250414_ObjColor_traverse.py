            ...
            if ask_type == "ordered_choice":
                for i in range(4):
                    ...
                    queries.append({
                        ...
                        'object_index': sam_id,  # 由 sam_id 可定位 query 所属的 question
                        'sam': sam,
                        'prompt': prompt,
                        'options': reordered,
                        'gt': gt,
            ...
            if ask_type == "ordered_choice":
                for i in range(4):
                    ...
                    queries.append({
                        ...
                        'object_index': sam_id,  # 由 q_idx 可定位 query 所属的 question
                        'sam': sam,
                        'prompt': prompt,
                        'options': reordered,
                        'gt': gt_color,
...
def group_per_question(all_queries, all_raw_ans, all_acc, all_invalid_info, dataset):
    ...
    for img_id in grouped:
        ...
        for (qtype, obj_index, gt), query_list in grouped[img_id].items():
            ...
            confuser = [opt for opt in query_list[0]['options'] if opt != gt]
